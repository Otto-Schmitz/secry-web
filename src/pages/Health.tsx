...
export default function Health() {
  ...
  const allergyUpdate = useMutation({
    mutationFn: (id: string) => allergyApi.update(id, { name: allergyForm.name, severity: (allergyForm.severity || undefined) as AllergySeverity | undefined, notes: allergyForm.notes || undefined }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['allergies'] }); qc.invalidateQueries({ queryKey: ['health'] }); setAllergyOpen(false); toast.success('Alergia atualizada'); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const openAllergyCreate = () => {
    setEditingAllergyId(null);
    setAllergyForm({ name: '', severity: '', notes: '' });
    setAllergyOpen(true);
  };

  const openAllergyEdit = (a: AllergyListItemResponse) => {
    setEditingAllergyId(a.id);
    setAllergyForm({ name: a.name, severity: a.severity || '', notes: a.notes || '' });
    setAllergyOpen(true);
  };

  const handleAllergySubmit = () => {
    if (editingAllergyId) {
      allergyUpdate.mutate(editingAllergyId);
    } else {
      allergyCreate.mutate();
    }
  };
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-8">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between px-1">
          ...
          <Button size="sm" variant="ghost" onClick={openAllergyCreate}>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between px-1">
          ...
          <Button size="sm" variant="ghost" onClick={openMedCreate}>
            <Plus className="h-4 w-4 mr-1" /> Adicionar
          </Button>
        </div>
        ...
        <SheetContent side="bottom" className="rounded-t-3xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleAllergySubmit(); }} className="space-y-4 mt-4">
            ...
            <div className="flex gap-3">
              ...
              <Button type="submit" className="w-full rounded-xl h-12" disabled={isAllergyPending}>
                {editingAllergyId ? 'Salvar' : 'Adicionar'}
              </Button>
            </div>
          </form>
        </SheetContent>
        ...
        <SheetContent side="bottom" className="rounded-t-3xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleMedSubmit(); }} className="space-y-4 mt-4">
            ...
            <div className="flex gap-3">
              ...
              <Button type="submit" className="w-full rounded-xl h-12" disabled={isMedPending}>
                {editingMedId ? 'Salvar' : 'Adicionar'}
              </Button>
            </div>
          </form>
        </SheetContent>
